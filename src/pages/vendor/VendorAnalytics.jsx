...
const VendorAnalytics = () => {
  ...
  const fetchVendorAnalytics = async () => {
    try {
      setLoading(true);
      const response = await apiService.getVendorAnalytics({ timeframe: selectedTimeframe });
  ...
  const generateMockAnalytics = () => ({
    revenue: {
      ...
      daily: Array.from({ length: 30 }, () => Math.floor(Math.random() * 500) + 100)
    },
    orders: {
      total: 234,
      pending: 12,
    ...
    try {
      ...
      switch (reportType) {
        case 'revenue':
          ...
          filename = `vendor-revenue-${selectedTimeframe}-${new Date().toISOString().split('T')[0]}.csv`;
          break;
        case 'products':
          csvContent = [
          ...
          filename = `vendor-products-${new Date().toISOString().split('T')[0]}.csv`;
          break;
        default:
          return;
      }
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <Link to="/vendor/dashboard" className="text-blue-600 hover:text-blue-700 mr-3">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-sm text-green-600">+{analytics?.revenue?.growth}% growth</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-sm text-blue-600">+{analytics?.orders?.growth}% growth</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-sm text-purple-600">{analytics?.products?.views?.toLocaleString()} views</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-sm text-orange-600">{analytics?.customers?.returning} returning</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="h-64 bg-gradient-to-br from-green-50 to-emerald-100 rounded-lg flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500">
                  Current: ${analytics?.revenue?.monthly?.slice(-1)[0]?.toLocaleString()}
                </p>
                <div className="mt-4 grid grid-cols-7 gap-1">
                  {analytics?.revenue?.monthly?.map((value, index) => (
                    <div
                      ...
                      style={{ height: `${(value / Math.max(...analytics.revenue.monthly)) * 40}px` }}
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              {[
                { status: 'Completed', count: analytics?.orders?.completed, color: 'bg-green-500' },
                { status: 'Pending', count: analytics?.orders?.pending, color: 'bg-yellow-500' },
                { status: 'Cancelled', count: analytics?.orders?.cancelled, color: 'bg-red-500' }
              ].map((item, index) => (
                <div key={index} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center">
                    <span className="font-medium text-gray-900 mr-2">{item.count}</span>
                    <div className="w-20 bg-gray-200 rounded-full h-2">
                      <div 
                        className={`h-2 rounded-full ${item.color}`}
                        style={{ width: `${(item.count / analytics?.orders?.total) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
              ...
              <div key={product._id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="text-sm text-gray-600">
                        {typeof product.rating === 'object' ? product.rating.average?.toFixed(1) : product.rating?.toFixed(1)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{Math.floor(Math.random() * 500) + 100}</p>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-gray-900">{analytics?.customers?.total}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Returning Customers</span>
                <span className="font-bold text-green-600">{analytics?.customers?.returning}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">New Customers</span>
                <span className="font-bold text-blue-600">{analytics?.customers?.newCustomers}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Satisfaction Rate</span>
                <div className="flex items-center">
                  <Star className="h-4 w-4 text-yellow-400 fill-current mr-1" />
                  <span className="font-bold text-gray-900">{analytics?.customers?.satisfaction}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                <div className="text-3xl font-bold text-green-600 mb-2">
                  ${(analytics?.revenue?.total / analytics?.orders?.total || 0).toFixed(2)}
                </div>
              ...
              <div className="text-center">
                <div className="text-3xl font-bold text-purple-600 mb-2">
                  {((analytics?.orders?.completed / analytics?.orders?.total) * 100 || 0).toFixed(1)}%
                </div>
