...
const VendorDashboard = () => {
  const { user } = useAuth();
  const { products, getProductsByVendor, deleteProduct } = useProducts();
  const [activeTab, setActiveTab] = useState('overview');
  
  const vendorProducts = getProductsByVendor(user?.vendorId);
                  ...
                  {[1, 2, 3].map((order) => (
                    <div key={order} className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">2 items • $299.99</p>
                      </div>
                  ...
                  {vendorProducts.slice(0, 3).map((product) => (
                    <div key={product.id} className="flex items-center">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-medium text-gray-900">{product.reviews} reviews</p>
            ...
            <div className="p-6">
              {vendorProducts.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 mb-6">Start by adding your first product to the store.</p>
                  <button className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">
                    <Plus className="h-4 w-4 mr-2" />
